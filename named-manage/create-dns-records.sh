...
v_fw_zone="$v_zone_dir/muthuks.local.lab-forward.db"
v_ptr_zone_prod="$v_zone_dir/192.168.168.muthuks.local.lab-reverse.db"
v_ptr_zone_test="$v_zone_dir/10.10.10.muthuks.local.lab-reverse.db"
v_ptr_zone_dev="$v_zone_dir/172.16.16.muthuks.local.lab-reverse.db"
...
f_update_dns_records() {
	...
	do
		...
		elif [[ $v_num_ptr -ge 234 && $v_num_ptr -le 254 ]]   #<---Ignoring IP Ranges reserved for DHCP
		then
			continue
			...
			then
				...
				echo -e "\nNote : IPs from $v_net_part.234 to $v_net_part.254 are reserved for DHCP lease." 
				exit
			fi
			break
		fi
	...
	if [[ $v_digits -eq 1 ]]
	then
		v_add_ptr_record=$(echo "$v_ptr_ip   IN PTR $v_set_record.muthuks.local.lab.")
...
f_main_menu() {
	v_input_host=$1
	v_input_network=$2
	echo -e "\n<< This script provitions fqdn from muthuks.local.lab domain >>\n\nNote: A and PTR record updates are done automatically\n"
